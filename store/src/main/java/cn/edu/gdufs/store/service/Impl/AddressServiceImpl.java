...
import cn.edu.gdufs.store.mapper.AddressMapper;
...
public class AddressServiceImpl implements AddressService {
    ...
    public void addNewAddress(Integer uid, String username, Address address) {
        ...
        if (rows != 1) {
            // 是：抛出InsertException
            throw new InsertException("插入收货地址失败！");
        }
    }
    ...
    public void delete(Integer aid, Integer uid, String username) {
        ...
        if (rows1 != 1) {
            throw new CancelOrderException("删除收货地址失败！");
        }
        ...
        if (rows2 != 1) {
            // 是：抛出UpdateException
            throw new UpdateException("更新收货地址失败！");
        }
    }
